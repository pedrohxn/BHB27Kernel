...
inline static void smartass_update_min_max_allcpus(void) {
	...
	for_each_online_cpu(i) {
		...
		if (this_smartass->enable)
			smartass_update_min_max(this_smartass,this_smartass->cur_policy,state_suspended);
	}
}
...
define_one_global_rw(up_rate_us);
define_one_global_rw(down_rate_us);
define_one_global_rw(sleep_ideal_freq);
define_one_global_rw(sleep_wakeup_freq);
define_one_global_rw(awake_ideal_freq);
define_one_global_rw(sample_rate_jiffies);
define_one_global_rw(ramp_up_step);
define_one_global_rw(ramp_down_step);
define_one_global_rw(max_cpu_load);
define_one_global_rw(min_cpu_load);
define_one_global_rw(boost_enabled);
define_one_global_rw(boost_pulse);

static struct attribute * smartass_attributes[] = {
	&debug_mask.attr,
	&up_rate_us.attr,
	&down_rate_us.attr,
	&sleep_ideal_freq.attr,
	&sleep_wakeup_freq.attr,
	&awake_ideal_freq.attr,
	&sample_rate_jiffies.attr,
	&ramp_up_step.attr,
	&ramp_down_step.attr,
	&max_cpu_load.attr,
	&min_cpu_load.attr,
		...
		unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		smartass_update_min_max(this_smartass,new_policy,state_suspended);
